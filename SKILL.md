...
| 类型 | 触发条件 | 关键词 |
|---|---|---|
| **类型1: 备案** | 20日→披露, 60日→报告证监会+提方案+持有人大会 | `60个工作日` + `报告证监会` + `解决方案` + `持有人大会`（无10日/6月时限） |
| **类型2: 备案+6个月大会** | 60日后的同一条款链同时包含10日内报告和6个月内召集大会 | `60个工作日` + `10个工作日内报告证监会` + `6个月内召集持有人大会` |
| **类型3: 自动触发终止** | 50日→自动终止且无需大会 | `50个工作日` + `自动终止` + `无需大会` |
...
```python
# 定位: "5000万元/五千万元" → 同一句基础条款含"份额持有人"和"200人/二百人"
# 基础门槛: 必须同时匹配20个工作日、200人、5000万元
# 阶段: stage1仅阿拉伯数字；stage2增加中文数字；stage3增加OCR分隔符容错
# 判定（特征必须位于对应60日/50日后的同一句条款）:
if 50日 and 自动终止 and 无需召开持有人大会 → 类型3
elif 60日 and 10日内报告证监会 and 6个月内召集大会 → 类型2
elif 60日 and 报告证监会 and 提出解决方案 and 召开大会 and 无上述时限 → 类型1
```
